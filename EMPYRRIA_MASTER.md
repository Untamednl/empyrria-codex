...
**Status:** **LOCKED (2026-05-14)** — **`src/pages/sigils/[slug].astro`** **presentation only** (**Tailwind** + markup order). **No** **`sigils.json`**, **schema**, **`.pages.yml`**, **Decap**, **validator**, **routing**, **`SigilCard`**, **Doctrine** chapter typography, **editorial** band, **registry** sections, or new image fields.

**Purpose:** **Zierota** feedback — **small** thumbnails on **`/sigils`** browse; **large** primary mark on detail **`/sigils/[slug]`** beneath **title + hero metadata**, in a **dark codex frame**, without duplicating a **tiny** icon above **`h1`**. (**Asset URL** for the plate is **`mainImage` ?? `image`** after **SIGIL DUAL IMAGE SYSTEM — PHASE 1**.)

**Authoritative problem statement, layout order, validation targets, and rollback:** **`EMPYRRIA_CODEX_MASTER.md`** → **SIGIL DETAIL MAIN IMAGE WINDOW — PHASE 1**.

### Governance rules (MASTER — main image Phase 1)

- **Browse unchanged** — **`SigilCard`** thumbnail sizes and **`/sigils`** grid **not** edited in this phase.  
- **Doctrine remains apex text** — image is **atmosphere + identity plate**; **no** competition with **Doctrine** serif/amber treatment.  
- **Frontend-only** — detail **`src`** per **SIGIL DUAL IMAGE SYSTEM — PHASE 1** (**`mainImage` ?? `image`**); **no** upload pipeline change beyond existing **`public/media/sigils`** contract.

### No-behavior-change statement (MASTER — main image Phase 1)

- **No** routes — **detail-page markup + Tailwind**; **image contract** extended in **SIGIL DUAL IMAGE SYSTEM — PHASE 1** (optional **`mainImage`**).

---

## SIGIL DUAL IMAGE SYSTEM — PHASE 1

**Status:** **LOCKED (2026-05-14)** — **Additive data + CMS + validation + detail rendering.** Optional **`mainImage`** on each sigil; **`image`** remains the **small** browse/card/showcase asset; **`/sigils/[slug]`** detail plate uses **`mainImage`** when set, else **`image`**. **`src/data/sigils.json`**: **no** required bulk migration — omit **`mainImage`** until needed.

**Purpose:** **Zierota** can maintain **two** image assets in **PATH A** (Pages CMS) and **Decap**: overview thumbnail vs large detail mark, with strict **`/media/sigils/`** validation for both.

**Authoritative field contract, PATH A / Decap exposure, validation, fallback, and rollback:** **`EMPYRRIA_CODEX_MASTER.md`** → **SIGIL DUAL IMAGE SYSTEM — PHASE 1**.

### Governance rules (MASTER — dual image Phase 1)

- **`SigilCard`**, **`/sigils`**, **`/showcase`** stay on **`image`** only — **no** **`mainImage`** on card surfaces unless a **future** lock says otherwise.  
- **Detail** resolves **`trim(mainImage) || trim(image)`** — backward compatible when **`mainImage`** is absent.  
- **Decap + Identity** remain **fallback** — **`public/admin/config.yml`** updated alongside **`.pages.yml`**.

### Rollback (MASTER — dual image Phase 1)

**Revert** the implementing commit (schema, types, validator, **`.pages.yml`**, Decap, **`[slug].astro`**, masters). JSON may retain or drop optional **`mainImage`** keys; code rollback restores single-field behavior only if validator/schema match.
...
- **Sigil detail main image window (Phase 1):** **`[slug].astro`** — large framed plate after hero metadata — **`## SIGIL DETAIL MAIN IMAGE WINDOW — PHASE 1`**. **Sigil dual image system (Phase 1):** optional **`mainImage`**; detail **`mainImage` ?? `image`**; cards/showcase stay on **`image`** — **`## SIGIL DUAL IMAGE SYSTEM — PHASE 1`**.
