...
**Purpose:** **Zierota** feedback — **small** thumbnails on **`/sigils`** browse; **large** primary **`sigil.image`** on detail **`/sigils/[slug]`** beneath **title + hero metadata**, in a **dark codex frame**, without duplicating a **tiny** icon above **`h1`**.
...
- **Frontend-only** — same **`sigil.image`** URL as before; **no** upload pipeline, **no** CMS config.

### No-behavior-change statement (MASTER — main image Phase 1)

- **No** data contracts, **no** routes, **no** admin — **detail-page markup + Tailwind only**.
...
- **Sigil detail main image window (Phase 1):** **`[slug].astro`** — large framed **`sigil.image`** after hero metadata; **`SigilCard`** / **`/sigils`** unchanged — **`## SIGIL DETAIL MAIN IMAGE WINDOW — PHASE 1`**; **no** schema, CMS, data, **Doctrine** typography, or routing changes.
